...
if [ "${PULLSUZ:-0}" != "1" ]; then
  ...
  if [ -n "$(sudo -u byd git status --porcelain --untracked-files=no)" ]; then
    ...
    sudo -u byd git status --short --untracked-files=no
    echo
  ...
  YENI_DOSYA=$(sudo -u byd git ls-files --others --exclude-standard | head -5)
  if [ -n "$YENI_DOSYA" ]; then
    echo "ℹ️  Depoya eklenmemiş dosyalar var (dağıtımı etkilemez):"
    echo "$YENI_DOSYA" | sed 's/^/     /'
  fi

  ONCEKI=$(sudo -u byd git rev-parse HEAD)
  echo "⬇️  GitHub'dan kod alınıyor..."
  sudo -u byd git pull --ff-only
  YENI=$(sudo -u byd git rev-parse HEAD)

  if [ "$ONCEKI" = "$YENI" ]; then
    echo "✔️  GitHub'da yeni bir şey yok."
  else
    DEGISEN=$(sudo -u byd git diff --name-only "$ONCEKI" "$YENI")
    ...
    if echo "$DEGISEN" | grep -q '^composer\.lock$'; then
      ...
      sudo -u byd composer install --no-interaction --optimize-autoloader
    fi
